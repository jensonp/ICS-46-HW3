    ...
    : public SortedList
{
    ...
    int binary_search(const string& word, int low, int high);
public:
    ...
    public:
        ...
        iterator& operator--() { --ptr; return *this; }
        iterator operator++(int) { iterator copy{ptr}; ++ptr; return copy; }
        iterator operator--(int) { iterator copy{ptr}; --ptr; return copy; }
        iterator& operator+=(difference_type const d) { ptr += d; return *this; }
        iterator& operator-=(difference_type const d) { ptr -= d; return *this; }
        friend iterator operator+(iterator i, difference_type const d) { i += d; return i; }
        friend iterator operator-(iterator i, difference_type const d) { i -= d; return i; }
        friend difference_type operator-(iterator const i, iterator const j) { return i.ptr - j.ptr; }
